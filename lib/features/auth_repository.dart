...
class AuthRepository {
  Future<bool> login(String username, String password) async {
    final response = await http.post(
      Uri.parse("http://172.16.0.238:8080/authen"), //nhớ đổi IP nếu cần
  ...
  Future<bool> register({
    ...
    final response = await http.post(
      Uri.parse("http://172.16.0.238:8080/api/v1/users/signup"), //10.0.2.2
      headers: {"Content-Type": "application/json"},
      body: jsonEncode({
        "username": username,
        "password": password,
  ...
  Future<void> logoutFlow(String token) async {
    ...
    await http.post(
      Uri.parse("http://172.16.0.238:8080/authen/logout"),
      headers: {"Content-Type": "application/json"},
  ...
  Future<bool> loginWithGoogle(String idToken) async {
    final response = await http.post(
      Uri.parse("http://172.16.0.238:8080/autheng/api/v1/auth/google"),
      headers: {"Content-Type": "application/json"},
